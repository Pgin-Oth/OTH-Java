...
public class ChatGui extends JFrame implements ActionListener, WindowListener, IMessageGui {
	...
	public ChatGui(IMessageSender sender) {
		...
		if(username != null && !"".equals(username))
			sender.openChatConnection(username, registryHost, bindingName, this);
		else
			System.exit(NORMAL);
	...
	class AnmeldeDialog extends JDialog {
		...
		public AnmeldeDialog(JFrame parent, String title, boolean modal) {
			...
			bindingNameField = new JTextField();
			bindingNameField.setEditable(true);
			bindingNameField.setColumns(12);
			panel.add(bindingNameField);
			...
			anmelden.addActionListener( new ActionListener(){
				@Override
				public void actionPerformed(ActionEvent e) {
					registryHost = hostField.getText();
					bindingName = bindingNameField.getText();
